...
Stage recording revalidates the complete aggregate snapshot and the dispatched prompt digest before accepting a nonempty report. A first successful record adds `report_sha256`, `completed_at`, and an `orchestrator-attested` binding; primary, peer, and final-diff records cannot be overwritten. Every recorded stage requires nonempty `tool`, `model`, and `session_id`. Dispatch and session IDs must be audit-unique. Exact copied primary/peer/final-diff report bytes fail finalization. Peer cannot be recorded with a completion time before primary; final-diff and focused verification require completed primary and peer stages and cannot predate either prerequisite. These checks run before the immutable reviewer record is written. Finalization independently requires valid nondecreasing primary, peer, and final-diff completion timestamps, requires every supplemental verifier to complete no earlier than peer, and requires every fixed or supplemental reviewer prompt and report path to equal its dispatched `audit.artifacts` entry before checking bytes and digests.
